...
% Affiliations
\def \USZ {University Hospital Zurich, Rämistrasse 100, 8091 Zurich, Switzerland}
\def \UZH {University of Zurich, Rämistrasse 71, 8006 Zurich, Switzerland}
\def \ISB {Inselspital Bern, Freiburgstrasse 18, 3010 Bern, Switzerland}
\def \CLB {Centre L\'eon B\'erard, 28 Rue Laennec, 69008 Lyon, France}
\def \KS  {Klinikum Stuttgart, Kriegsbergstraße 60, 70174 Stuttgart, Germany}
...
The dataset containing the consensus decision for the involvement of each level in every patient was then used for model parameter learning. We assumed that it represents an observation of the true hidden state $\boldsymbol{\xi}_k$, which corresponds to setting the sensitivity and specificity both to one. The frequencies of some of the most important combinations of involved lymph node levels are listed in \autoref{table:data_prevalence}.
