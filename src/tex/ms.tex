...
To determine the optimal graph describing the data at hand best, we considered six models, each with one of the green candidate arcs of \cref{fig:extended_graph} added to the base graph. Every one of these models was evaluated by computing its evidence via thermodynamic integration as described below in \cref{subsec:complete_model:comparison}. These evidences are shown, among others, in \cref{table:evidence}. Promising combination with two added edges were then considered in a combined model which was ultimately designated to be the "winning graph" which is in great detail compared to the base graph in the results (\cref{sec:results}). All considered graphs' performances are visualized in \cref{fig:ranking}.
...
The model evidences of the six models that were based on graphs with one additional arc compared to the base model's are plotted in \cref{table:evidence}. A visual ranking is provided in \cref{fig:ranking}. Evidently, adding a connection from \gls{lnl} I to II is strongly supported given this dataset and slightly beats the reverse connection from \gls{lnl} II to I. Similarly, there is strong evidence for introducing an arc from \gls{lnl} IV to V. Furthermore, there is substantial evidence for an arc from \gls{lnl} III to V. All other investigated additions lead to improvements that are at best barely worth a mention or do not justify the additional complexity at all, indicated by a lower evidence.

Based on this table, we determined the final candidates for the optimal graph should be combinations of the graphs with the added connections from level I to II, III to V, and IV to V. For the three resulting graphs we again computed the model evidence and added it to the table in \cref{table:evidence}. The best performing graph with decisive evidence over the base graph turned out to be the one which combines the arcs from \gls{lnl} I to II and IV to V. Interestingly, the evidence gain of this "winning graph" is roughly the sum of the gains seen in the two candidates where only one of these connections was added, respectively. This indicates that the two additional parameters are largely independent of each other and manage to describe different aspects of the data.
...
\begin{figure}
    ...
    \begin{centering}
        ...
        \caption{Predictions for the prevalence of \gls{lnl} VII involvement based on the base graph (hatched) and the winning graph (blue histogram) plotted against the beta posterior over the prevalence in the data (solid line). This is shown for early T-category (left panel) and late T-category (right panel). Since no connections are added to or from level VII, no change in the model's ability to describe this \gls{lnl}'s involvement is expected.}
        \label{fig:VII_prevs}
    \end{centering}
\end{figure}
