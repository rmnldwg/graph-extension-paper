...
{\it Contralateral Spread:} So far, our model concerns itself with the ipsilateral lymphatic spread. However, our data suggests that at least contralateral \gls{lnl} II is frequently involved in \gls{opscc} patients. It is also of interest clinically as it presents the opportunity for further reducing the \gls{ctv-n} when the predicted risk is low enough. Therefore extending the model to include the contralateral side could be an important next step on its way to a clinical validation and ultimately implementation.
