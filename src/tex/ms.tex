...
The term $P \big( X_v = \xi_{iv} \mid \left\{ X_r = \xi_{jr} \right\}_{r \in \operatorname{pa}(v)} \big)$ in \cref{eq:transition_prob} implicitly depends on how we parameterize the arcs of the underlying \gls{dag}. If we use the graph displayed in \cref{fig:graph_with_obs}), we can write down a conditional probability table for the probability of $X_3$ being healthy or metastatic, based on the state of the parent \gls{lnl} $\operatorname{pa}(3) = 2$:
